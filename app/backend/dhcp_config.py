"""
Full DHCP server configuration generator for PXE Boot Station.

Generates complete DHCP server config files (ISC DHCP, dnsmasq, MikroTik)
including subnet/netmask/DNS/lease settings.

Note: for PXE-specific snippets to add to an existing DHCP server,
see dhcp_helper.py (DHCPConfig / DHCPConfigGenerator).
"""
...
class DHCPServerConfigValidator:
    ...
    def validate_config(cls, config: DHCPServerConfig) -> Tuple[bool, List[str]]:
...
class DHCPServerConfigGenerator:
    """DHCP configuration file generators"""

    @staticmethod
    @safe_operation("ISC DHCP configuration generation")
    def generate_isc_dhcp_config(config: DHCPServerConfig) -> str:
...
"""
    ...
    def generate_dnsmasq_config(config: DHCPServerConfig) -> str:
...
"""
    ...
    def generate_mikrotik_config(config: DHCPServerConfig) -> str:
...
class DHCPServerConfigManager:
    """Main DHCP configuration management class"""

    def __init__(self):
        self.validator = DHCPServerConfigValidator()
        self.generator = DHCPServerConfigGenerator()
    ...
    def validate_and_generate(
        self, config: DHCPServerConfig, config_type: str = "isc"
    ) -> Tuple[bool, str, str]:
