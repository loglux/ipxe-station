...
class TFTPTester:
    ...
    def test_tftp_connection(
        host: str = "localhost", port: int = 69, filename: str = "undionly.kpxe", timeout: int = 5
    ) -> str:
        ...
        try:
            ...
            try:
                result = subprocess.run(
                    ["netstat", "-ulnp"], capture_output=True, text=True, timeout=5
                )
...
class SystemTester:
    ...
    def run_full_system_test(self) -> str:
        ...
        if "Found" in ipxe_file_result:
            ipxe_http_result = self.http_tester.test_endpoint(
                "http://localhost:8000/ipxe/boot.ipxe"
            )
    ...
    def _generate_final_status_with_ubuntu(self, test_results: List[str]) -> List[str]:
        ...
        ubuntu_versions_working = any(
            "Working Ubuntu versions" in result for result in test_results
        )
        tftp_working = any("TFTP test: SUCCESS" in result for result in test_results)
        ipxe_files_present = any(
            "undionly.kpxe" in result and "Found" in result for result in test_results
        )
