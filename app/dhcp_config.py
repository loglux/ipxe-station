...
class DHCPConfigValidator:
    ...
    def validate_dns_servers(dns_list: List[str]) -> Tuple[bool, str]:
        ...
        invalid_dns = []
        for dns in dns_list:
            is_valid, _ = DHCPConfigValidator.validate_ip_address(dns)
            if not is_valid:
    ...
    def validate_config(cls, config: DHCPConfig) -> Tuple[bool, List[str]]:
        ...
        # Validate server IP
        is_valid, msg = cls.validate_ip_address(config.server_ip)
        if not is_valid:
            errors.append(f"Server IP: {msg}")

        # Validate router IP
        is_valid, msg = cls.validate_ip_address(config.router_ip)
        if not is_valid:
        ...
        # Validate TFTP server
        if config.tftp_server:
            is_valid, msg = cls.validate_ip_address(config.tftp_server)
            if not is_valid:
...
"""
    ...
    def generate_dnsmasq_config(config: DHCPConfig) -> str:
        ...
        dhcp_range_start, dhcp_range_end = _calculate_dhcp_range(config.subnet, config.netmask).split(' ')
...
class DHCPConfigManager:
    ...
    def create_config(self, server_ip: str, subnet: str, netmask: str,
                      router_ip: str, dns_servers: List[str], **kwargs) -> DHCPConfig:
    ...
    def validate_and_generate(self, config: DHCPConfig,
                              config_type: str = "isc") -> Tuple[bool, str, str]:
...
def create_simple_config(server_ip: str, network_cidr: str,
                         dns_servers: Optional[List[str]] = None) -> DHCPConfig:
    ...
    try:
        ...
        raise ValueError(f"Invalid network CIDR: {network_cidr} - {str(e)}")